...
n_perfis = 6
perfis_cfg = pd.read_csv('perfispv_cfg.csv',header = 0, sep = ';')

for perfil in range(n_perfis):
#Configurações da Simulação
    replicacao=5#input("Insira o numero de replicacoes...")
    ...
    geracoes=20#input('Quantas geracoes voce deseja...')
    ...
    listdes=w[(w.index(perfis_cfg['desspeeddist_min'][perfil])):(w.index(perfis_cfg['desspeeddist_max'][perfil])+1)]
    #print(listdes)
    ...
    for x in range(ind):
        ...
        bxmult[x]=round(random.uniform(perfis_cfg['bxmult_min'][perfil],perfis_cfg['bxmult_max'][perfil]),1)
        bxadd[x]=round(random.uniform(perfis_cfg['bxadd_min'][perfil],perfis_cfg['bxadd_min'][perfil]),1)
        ax[x]=round(random.uniform(perfis_cfg['ax_min'][perfil],perfis_cfg['ax_max'][perfil]),1)
        sleepdur[x]=round(random.uniform(perfis_cfg['sleepdur_min'][perfil],perfis_cfg['sleepdur_max'][perfil]),1)
        sleepprob[x]=round(random.uniform(perfis_cfg['sleepprob_min'][perfil],perfis_cfg['sleepprob_max'][perfil]),3)
        minheadw[x]=round(random.uniform(perfis_cfg['minheadw_min'][perfil],perfis_cfg['minheadw_max'][perfil]),1)
        safedist[x]=round(random.uniform(perfis_cfg['safedist_min'][perfil],perfis_cfg['safedist_max'][perfil]),1)
    ...
    def simulacao(bxadd,seed0,ax,delta_seed,rep,bxmult,g,i,desspeeddist,sleepdur,sleepprob,minhdwy,safdist): #Estabelecendo uma funcao para a simulacao
        ...
        for l in range(replicacao):
            ...
            print('geracao %.1f, individuo %.1f, replicacao %.1f \n' % (g,i,l))
            ...
            appended_data.to_csv('Dados_raw_%d.csv' % perfil,sep=';',float_format='%.3f') 
            #-------------------------------------------------------
           ...
           #print(arrumadosbuffer)
            ...
            if g == (geracoes-1):
                ...
                appended_arrumados.to_csv('Dados_Ordenados_%d.csv' % perfil,sep=';',float_format='%.3f')
            #-------------------------------------------------------
            print(time.ctime()+ "\n")
    ...
    for geracao in range(geracoes-1):
            ...
            for q in range(ind): 
                ...
                if q!=indivm:
                    ...
                    if q in lista_erro_maiores:
                        df_inputs['bxmult'][q]=round(random.uniform(perfis_cfg['bxmult_min'][perfil],perfis_cfg['bxmult_max'][perfil]),1)
                        df_inputs['bxadd'][q]=round(random.uniform(perfis_cfg['bxadd_min'][perfil],perfis_cfg['bxadd_max'][perfil]),1)
                        df_inputs['ax'][q]=round(random.uniform(perfis_cfg['ax_min'][perfil],perfis_cfg['ax_max'][perfil]),1)
                        df_inputs['desspeeddist'][q]=random.choice(listdes)#meme choose
                        df_inputs['sleepdur'][q]=round(random.uniform(perfis_cfg['sleepdur_min'][perfil],perfis_cfg['sleepdur_max'][perfil]),1)
                        df_inputs['sleepprob'][q]=round(random.uniform(perfis_cfg['sleepprob_min'][perfil],perfis_cfg['sleepprob_max'][perfil]),3)
                        df_inputs['minheadw'][q]=round(random.uniform(perfis_cfg['minheadw_min'][perfil],perfis_cfg['minheadw_max'][perfil]),1)
                        df_inputs['safedist'][q]=round(random.uniform(perfis_cfg['safedist_min'][perfil],perfis_cfg['safedist_max'][perfil]),1)
        ...
        alfas = pd.concat(lista_agregacao_alfa,ignore_index=True)
        aresumo = pd.concat(lista_agregacao_resumido,ignore_index = True)
        aresumo.to_csv('Dados_light_%d.csv' % perfil ,sep=';',float_format='%.8f')
        alfas.to_csv('Dados_Alfas_%d.csv' % perfil,sep=';',float_format='%.8f')    
        
    sheet_name = 'GraficodosAlfasPV'
    writer     = pd.ExcelWriter('Progressao_dos_Alfas_%d.xlsx' % perfil, engine='xlsxwriter')
    ...
    writer.save()
    print()
    print("Perfil %d simulado em " % perfil + "%.1f segundos \n" % (time.time()-start_time))
